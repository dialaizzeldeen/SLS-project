...
public class profileBackground extends AsyncTask<String,Void,String> {
    ...
    protected String doInBackground(String... voids) {
        ...
        String save_url="http://192.168.137.1/profailPage.php"; //to conect with local host
        if(type.equals("save")){
            try {
                String username =voids[1]; //get the username value from the edittext
                String gender =voids[2];
                String location =voids[3]; //get the password value from the edittext
                String bday =voids[4];
                String mobile =voids[5];
                String personalemail =voids[6];
                ...
                String post_data= URLEncoder.encode("usernameK","UTF-8")+"="+URLEncoder.encode(username,"UTF-8")+"&"
                        +URLEncoder.encode("genderK","UTF-8")+"="+URLEncoder.encode(gender,"UTF-8")+"&"+
                        URLEncoder.encode("locationK","UTF-8")+"="+URLEncoder.encode(location,"UTF-8")+"&"+
                        URLEncoder.encode("bdayK","UTF-8")+"="+URLEncoder.encode(bday,"UTF-8")+"&"+
                        URLEncoder.encode("mobileK","UTF-8")+"="+URLEncoder.encode(mobile,"UTF-8")+"&"+
                        URLEncoder.encode("personalemailK","UTF-8")+"="+URLEncoder.encode(personalemail,"UTF-8");
